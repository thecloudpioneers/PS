################### SharePoint Online - Permission Report - All Items ###############
...
Function Generate-PnPListPermissionRpt()
{
    ...
    # Define credentials  (This is the section to update)
    $Email = "onedrivesharepointaudit@accesslex.org" # Replace with your auditing user email
    $Password = "!%j0m6lp!w/Wdd%!7q" # Replace with your auditing user password
 ...
 ($Email, $SecurePassword)
    ...
    Try {
        ...
        try {
            # Add the auditing user as a site collection admin
            Write-host "Adding audit user as Site Collection Admin..."
            Set-PnPSiteCollectionAdmin -Site $SiteURL -Emails $Email -ErrorAction Stop
        ...
        Write-host -f Green "`n*** Site Permission Report Generated Successfully!***"
            ...
            Write-host -f Red "Error processing site $($Site.Url): $($_.Exception.Message)"
        }
        finally {
            try {
                # Remove the auditing user from site collection admins
                Write-host "Removing audit user from Site Collection Admins..."
                Remove-PnPSiteCollectionAdmin -Site $Site.Url -Emails $Email -ErrorAction Stop
            }
            catch {
                Write-host -f Red "Error removing audit user from site collection admins for $($Site.Url): $($_.Exception.Message)"
            }
        }
...
#Read more: https://www.sharepointdiary.com/2019/09/sharepoint-online-user-permissions-audit-report-using-pnp-powershell.html#ixzz8mgZY6Vzh
